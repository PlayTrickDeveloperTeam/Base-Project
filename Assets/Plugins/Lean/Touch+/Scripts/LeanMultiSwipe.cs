...
namespace Lean.Touch
{
    ...
    public class LeanMultiSwipe : MonoBehaviour
    {
        ...
        public FingerListEvent OnFingers { get { if (onFingers == null) onFingers = new FingerListEvent(); return onFingers; } }

        [FSA("onSwipe")] [FSA("OnSwipe")] [SerializeField] private FingerListEvent onFingers;

        // Called when a multi-swipe occurs where each finger moves paralell to each other (Vector2 = ScaledDirection)
        public Vector2Event OnSwipeParallel { get { if (onSwipeParallel == null) onSwipeParallel = new Vector2Event(); return onSwipeParallel; } }

        [FSA("OnSwipeParallel")] [SerializeField] private Vector2Event onSwipeParallel;

        // Called when a multi-swipe occurs where each finger pinches in (Float = ScaledDistance)
        public FloatEvent OnSwipeIn { get { if (onSwipeIn == null) onSwipeIn = new FloatEvent(); return onSwipeIn; } }

        [FSA("OnSwipeIn")] [SerializeField] private FloatEvent onSwipeIn;

        // Called when a multi-swipe occurs where each finger pinches out (Float = ScaledDistance)
        public FloatEvent OnSwipeOut { get { if (onSwipeOut == null) onSwipeOut = new FloatEvent(); return onSwipeOut; } }

        [FSA("OnSwipeOut")] [SerializeField] private FloatEvent onSwipeOut;
